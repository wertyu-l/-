package com.example.demo.simulator2.core;
...
public class DeviceRepository {
    ...
    public SimDeviceInfo loadDeviceInfo() {
        return jdbc.queryForObject(
                "SELECT device_name, device_type, device_category, model, serial_number, " +
                        "input_channel_1, input_channel_2, max_resolution FROM DEVICE_INFO LIMIT 1",
                (rs, rowNum) -> {
    ...
    public SimDeviceCapability loadDeviceCapability() {
        return jdbc.queryForObject(
                "SELECT support_move, support_resize, support_overlay, max_resolution, " +
                        "input_channel_1, input_channel_2 FROM DEVICE_CAPABILITY LIMIT 1",
                (rs, rowNum) -> {
    ...
    public void updateCapability(SimDeviceCapability cap) {
        jdbc.update("UPDATE DEVICE_CAPABILITY SET support_move=?, support_resize=?, " +
                        "support_overlay=?, max_resolution=?, input_channel_1=?, input_channel_2=? WHERE id=1",
                cap.isSupportMove(), cap.isSupportResize(), cap.isSupportOverlay(),
                cap.getMaxResolution(), cap.getInputChannel1(), cap.getInputChannel2());
    }

    public void updateDeviceInfoFromCapability(SimDeviceCapability cap) {
        jdbc.update("UPDATE DEVICE_INFO SET input_channel_1=?, input_channel_2=?, max_resolution=? WHERE id=1",
                cap.getInputChannel1(), cap.getInputChannel2(), cap.getMaxResolution());
    }
